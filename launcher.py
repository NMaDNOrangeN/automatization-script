            ...
            for line in f:
                ...
                if line and '=' in line and not line.startswith(';'):
                    ...
                    if '|' in rest:
                        ...
                        ttype, path = 'gui', rest
                    tools.append({'name': name.strip(), 'type': ttype.strip(), 'path': path.strip(), 'desc': desc})
    return tools

def save_tools(tools):
    with open(CONFIG_FILE, 'w', encoding='utf-8') as f:
        for t in tools:
            f.write(f"{t['name']}={t['type']}|{t['path']}|{t.get('desc', '')}\n")
...
class App(tk.Tk):
    ...
    def build_run_tab(self):
        ...
        self.tree_run = ttk.Treeview(tree_frame, columns=('name', 'type', 'desc'), show='headings', selectmode='extended')        
        self.tree_run.heading('name', text='Название инструмента', anchor='w')
        self.tree_run.heading('type', text='Тип', anchor='w')
        self.tree_run.heading('desc', text='Описание', anchor='w')
        
        self.tree_run.column('name', width=250, anchor='w')
        self.tree_run.column('type', width=100, anchor='w')
        self.tree_run.column('desc', width=450, anchor='w')
    ...
    def refresh_tools_list(self):
        ...
        for t in self.tools:
            self.tree_run.insert('', 'end', values=(f"  {t['name']}", f"  {t['type'].upper()}", f"  {t.get('desc', '')}"))
    ...
    def build_editor_tab(self):
        ...
        ttk.Label(r_inner, text="Описание:").grid(row=3, column=0, sticky='w', pady=10)
        self.edit_desc_var = tk.StringVar()
        ttk.Entry(r_inner, textvariable=self.edit_desc_var, width=25).grid(row=3, column=1, sticky='ew', padx=(10, 0), pady=10)
        ...
        self.tree_editor = ttk.Treeview(tree_frame, columns=('name', 'type', 'path', 'desc'), show='headings')
        ...
        self.tree_editor.heading('desc', text='Описание', anchor='w')
        
        self.tree_editor.column('name', width=120, anchor='w')
        self.tree_editor.column('type', width=70, anchor='w')
        self.tree_editor.column('path', width=150, anchor='w')
        self.tree_editor.column('desc', width=180, anchor='w')
    ...
    def refresh_editor_list(self):
        ...
        for t in self.tools:
            self.tree_editor.insert('', 'end', values=(t['name'], t['type'], t['path'], t.get('desc', '')))
    ...
    def on_editor_select(self, event):
        ...
        if len(values) > 3:
            self.edit_desc_var.set(values[3])
        else:
    ...
    def add_tool(self):
        ...
        desc = self.edit_desc_var.get().strip()
        ...
        if any(t['name'] == name for t in self.tools):
            ...
            return
        self.tools.append({'name': name, 'type': ttype, 'path': path, 'desc': desc})
    ...
    def edit_tool(self):
        ...
        if any(t['name'] == new_name for i, t in enumerate(self.tools) if i != idx):
            messagebox.showerror("Ошибка", "Инструмент с таким именем уже существует.")
            return
        self.tools[idx] = {'name': new_name, 'type': new_type, 'path': new_path, 'desc': new_desc}
        save_tools(self.tools)
        self.refresh_editor_list()
        self.refresh_tools_list()
    ...
    def build_report_tab(self):
        ...
        ttk.Button(top_frame, text="💾 Сохранить и очистить логи", command=self.generate_report).pack(side='left')
    ...
    def generate_report(self):
        if not os.path.exists(LOG_FILE) or os.path.getsize(LOG_FILE) == 0:
            ...
            self.report_text.insert('1.0', "Нет данных для генерации отчёта. Сначала запустите инструменты.")
            return

        default_name = f"report_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
            ...
            for row in reader:
                if len(row) >= 4:
                    ...
                    if row[3].strip().upper() in ['FILE_NOT_FOUND', 'ERROR']:
                        ...
                        ok += 1
                    rows.append(row)
                    if len(row) > 4 and row[4] and os.path.exists(row[4]):
        ...
        self.status_var.set(f"Отчёт сохранён: {os.path.basename(save_path)}")

        for lf in log_files_to_delete:
            try:
                os.remove(lf)
            except Exception:
                pass
